# Отладка STM32F1/primGPT с Cortex-Debug

Полное руководство по отладке проекта primGPT_F1 (STM32F103C8T6 Blue Pill) в VS Code с расширением Cortex-Debug — от основ до продвинутых техник.
...
1. **Подключите ST-Link** к ПК и плате Blue Pill
...
⚠️ **Требует SVD файл** в `launch.json` (требует настройки).
...
```
RTOS (FreeRTOS)
  ├─ defaultTask (Running)       ← Текущая активная задача
  │   └─ Stack: 512/1024 bytes free  ⚠️ Хватает свободной памяти?
  ├─ LedTask (Blocked)
...
### Критическая проблема: переполнение стека на STM32F1

⚠️ **ВНИМАНИЕ**: На STM32F103C8T6 всего **20 KB RAM**. Каждый байт на счёту!

Мониторьте стек задач постоянно:
- `defaultTask` (1024 B): должна иметь минимум 200 B свободного стека
- `LedTask` (512 B): должна иметь минимум 100 B
- `Uart1Task` (2048 B): должна иметь минимум 300 B
...
### Сценарий 6: Переполнение стека (Stack Overflow) — КРИТИЧНО для STM32F1
...
3. Проверьте **Stack free** для каждой задачи:
   ```
   ├─ defaultTask
   │   └─ Stack: 20/1024 bytes free  ⚠️ КРИТИЧЕСКОЕ!
   ```
...
- **На STM32F1 это особенно важно!** Имеется только 14 KB FreeRTOS heap.
...
1. Добавьте в `launch.json`:
   ```json
   {
     ...
     "swoConfig": {
       "enabled": true,
       "cpuFrequency": 72000000,   // SYSCLK 72 MHz для STM32F1
...
**Решение:**
1. Скачайте `STM32F103xB.svd` из [CMSIS-SVD repo](https://github.com/posborne/cmsis-svd/blob/master/data/STMicro/STM32F1.svd)
2. Положите в корень проекта
3. В `launch.json` укажите путь:
   ```json
   "svdFile": "${workspaceFolder}/STM32F103xB.svd"
   ```
...
### 6. Мониторинг памяти на STM32F1

**КРИТИЧНО на STM32F1!** Добавьте в WATCH:
```
xPortGetFreeHeapSize()    // Свободная куча (в байтах)
xPortGetMinimumEverFreeHeapSize()  // Минимум когда-либо достигнутый
```

Если свободной памяти менее 200 B — опасно! Срочно оптимизируйте.
...
- **STM32F1 Reference Manual**: https://www.st.com/resource/en/reference_manual/cd00171190-stm32f101xx-stm32f102xx-stm32f103xx-stm32f105xx-and-stm32f107xx-advanced-armbased-32-bit-mcus-stmicroelectronics.pdf

---

## Адаптация для STM32F1 (важные отличия от F411CE)

### Архитектура
- **MCU**: Cortex-M3 (без FPU, как M4F в F411CE)
- **Тактовая частота**: 72 MHz (вместо 96 MHz)
- **USART для отладки**: USART3 (вместо USART6) — 115200 baud
- **Регистровая архитектура**: STM32F1xx отличается от F4xx

### Память (КРИТИЧНО!)
- **Total RAM**: 20 KB (вместо 128 KB)
- **FreeRTOS heap**: 14 KB (вместо 10 KB, но это больший процент от общей памяти!)
- **Используемо**: ~85% занято (вместо 41%)
- **Свободная RAM**: ~3-4 KB

### FreeRTOS задачи
| Задача | Размер | Status |
|--------|--------|--------|
| defaultTask | 1024 B | Требует близкого мониторинга |
| LedTask | 512 B | Минимальный размер |
| Uart1Task | 2048 B | Второй по величине потребитель |
| **Итого** | **3584 B** | **25% от всей RAM!** |

### Рекомендации для STM32F1
1. **Постоянно мониторьте стек** через RTOS View
2. Избегайте локальных буферов размером > 100 B
3. Используйте статические буферы, не динамические malloc()
4. Если стек задачи < 100 B свободных — срочно увеличивайте!
5. Проверяйте свободную кучу: должна быть > 500 B
