# Быстрый старт и отладка primGPT (STM32F103C8T6) в Cursor на macOS
...
| **Прошивка** | `Cmd+Shift+P` — `Tasks: Run Task` — Flash | Терминал: `openocd -f "primGPT Debug.cfg" -c "program Debug/primGPT.elf verify reset exit"` |
| **Сборка + прошивка** | `Cmd+Shift+P` — `Tasks: Run Task` — Build & Flash | Терминал: `make -C Debug all -j8 && openocd -f "primGPT Debug.cfg" -c "program Debug/primGPT.elf verify reset exit"` |
| **Отладка** | `F5` (сборка — прошивка — отладка) | |
| **Размер прошивки** | `Cmd+Shift+P` — `Tasks: Run Task` — Firmware Size | Терминал: `arm-none-eabi-size Debug/primGPT.elf` |
...
1. **Подключите ST-Link** к Mac и плате Blue Pill
