# Работа с проектом primGPT (STM32F103) в VS Code

Подробное руководство по редактированию, сборке и отладке STM32F1-проекта в Visual Studio Code.
...
Файл `.vscode/tasks.json` автоматически добавляет пути к toolchain из STM32CubeIDE, поэтому сборка будет работать сразу через **Ctrl+Shift+B** без ручной настройки системного PATH.
...
2. Найдите переменную `Path` и добавьте пути к вашей установке STM32CubeIDE
...
1. **Откройте проект в STM32CubeIDE**
   ...
   - Выберите папку проекта `primGPT`
...
3. **Проверьте результат**
   - В папке `Debug/` должны появиться:
     ...
     - `primGPT.elf` (прошивка)
     - `primGPT.hex`, `.bin`, `.map`, `.list`
  ...
  "configurations": [
    {
      ...
      "svdFile": "${workspaceFolder}/Drivers/CMSIS/Device/ST/STM32F1xx/Include/STM32F103.svd",
      "preLaunchTask": "Build",
...
⚠️ **Примечание**: если SVD файл отсутствует, удалите строку `"svdFile": ...` или скачайте [STM32F103.svd](https://github.com/posborne/cmsis-svd/blob/master/data/STMicro/STM32F103.svd).
...
# Чтение Flash памяти
openocd -f "primGPT Debug.cfg" -c "init; flash read_bank 0 flash_dump.bin 0 0x20000; exit"
```
...
**Решение:**
⚠️ **ВАЖНО**: STM32F103C8T6 имеет только 20 KB RAM! Это критическое ограничение.
1. Проверьте использование памяти: `arm-none-eabi-size Debug/primGPT.elf`
2. Смотрите карту памяти: `cat Debug/primGPT.map | grep -E "\.bss|\.data|\.rodata"`
3. Оптимизируйте:
